...
| [19](skills/LOOP-19-composting.md) | Composting | Knowledge & Finding Consolidation | read-only→branch | Distills noisy near-duplicate findings/ideas into a canonical component set, decides group-vs-recurse, and stamps a content signature for future dedup matching. |
| [20](skills/LOOP-20-idea-arena.md) | Idea Arena | Ideation & Convergent Synthesis | read-only | Generates diverse candidate ideas in parallel, runs them through one shared adversarial-debate-and-mutation arena, converges to a provenance-tracked synthesized idea, and packages it for agent handoff. |
